    ...
    pub fn compile_ir(
        ...
        match ir {
            ...
            IrNode::App(func, arg) => {
                ...
                let i64_type = self.context.i64_type();
                let fn_type = i64_type.fn_type(&[i64_type.into()], false);
                let call_site = self.builder.build_indirect_call(fn_type, compiled_func.into_pointer_value(), &[compiled_arg.into()], "calltmp").map_err(|e| e.to_string())?;
            ...
            IrNode::MkPair(left, right) => {
                ...
                let struct_type = self.context.struct_type(&[lhs.get_type(), rhs.get_type()], false);
                ...
                self.builder.build_store(ptr_0, lhs).map_err(|e| e.to_string())?;
                let ptr_1 = self.builder.build_struct_gep(struct_type, pair_ptr, 1, "p1").map_err(|e| e.to_string())?;
                self.builder.build_store(ptr_1, rhs).map_err(|e| e.to_string())?;
                Ok(pair_ptr.into())
            }
            IrNode::Left(pair) => {
                let pair_ptr = self.compile_ir(pair, env)?.into_pointer_value();
                let i64_type = self.context.i64_type();
            ...
            IrNode::ArraySwap(arr, idx, val) => {
                let ptr = self.compile_ir(arr, env)?.into_pointer_value();
            ...
            IrNode::Match(..) => {
                Err("Native Codegen for 'Match' (Sum Types) requires Tagged Union implementation - Coming soon!".to_string())
            }
        }
    }
}